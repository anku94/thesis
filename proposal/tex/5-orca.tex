...
\begin{figure}[h]
  ...
  shows random spikes in the volatile function delaying synchronization. Tuning the volatile function
  does not affects its average time, but leads to a 50\% reduction in their combined time. }
...
\begin{figure}[h]
  ...
  grid of ranks and timesteps. Tracing captures all event data, but writes it as separate per-rank streams
  to storage, posing analytical challenges. Profing and sampling offer arbitrary fixed reductions of data, obscuring
  insight. ORCA enables coordinated collection of timestep-wise telemetry from all ranks, with programmable
  in-situ aggregation.
  }
...
\begin{figure}[h]
  ...
  controller via a  hierarchy of aggregators, and optionally a tier of local lead ranks. Application ranks form
  the synchronous domain while ORCA executes asynchronously.}
...
\begin{itemize}
  \item \textbf{Tree-Based Overlay Network (TBON):}
        As shown in \cref{fig:orca-overlay}, ORCA organizes application ranks into a logical
        tree-based overlay with a controller at the root, a tier of aggregators, optionally a tier of
        lead ranks (aggregating within node groups), and application ranks as leaves.
...
absorbing these capabilities, enabling in-situ visualization workflows within the same pipeline.
Previous work has also shown how accelerators can be used for in-situ analysis on Arrow
