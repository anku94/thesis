...
Large-scale parallel programs—scientific simulations, distributed training—synchronize thousands of
processes in lockstep, which means even small inefficiencies compound into cluster-wide
bottlenecks.
Many of these inefficiencies are inherently unpredictable: they arise only at runtime and can only
be addressed through adaptation.
How quickly a system can observe a problem, characterize it, and respond defines its practical
efficiency.
This thesis argues that general-purpose infrastructure for such adaptive feedback loops is the
missing piece in today's parallel systems.
Two case studies—one with an online loop, one without—show the difference this makes and motivate
steerable observability: an always-on, programmable substrate for observing and responding to
runtime behavior.
We present ORCA, a system that realizes this paradigm with minimal overhead and collapses manual
investigation timelines by orders of magnitude.
