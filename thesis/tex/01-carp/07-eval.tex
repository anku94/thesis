...
We use a 32-node compute cluster with each node having two 8-core Intel Xeon E5-2670 CPUs and 64GB
DRAM.
The nodes are connected via 40Gbps Infiniband QDR.
Simulation output is written to a shared Lustre storage cluster consisting of 20 nodes identical to
those in the compute cluster, each having a 240GiB SSD.
...
Each timestep is 188GB of data, and the total simulation data is 2.2TB.
...
The sorted output uses 12MB SSTs and a manifest with one entry per SST, a format similar to KoiDB.
...
CARP is slower for highly selective queries (177ms vs 22ms for a query with 0.01\% selectivity)
...
(FastQuery and TritonSort) significantly reduces the effective application bandwidth from 3GB/s to
...
latency of a single round is only 150ms even at 2048 ranks.
