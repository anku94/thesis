...
\begin{itemize}[leftmargin=*]
  ...
  \item In-situ analytics (\S\ref{sec:orca:des_insitu}) and the OrcaFlow interface enable real-time
        ...
        configurable sinks.
  \item The control plane (\S\ref{sec:orca:des_ctl}) enables online interventions via a speculative variant
        of two-phase commit called \textsf{TS2PC} (timestep-linked two-phase commit).
...
\begin{itemize}[leftmargin=*]
  ...
  \item At rest, data is persisted as \emph{timestep-partitioned}
        Parquet~\cite{:ApacheParquet} (see \Cref{fig:orca:des-arch}) for efficient offline analyses.
        Formats such as OTF2~\cite{Eschw2012:OTF2} and JSON~\cite{:ChromeJsonTEF} --- used by conventional
        tracers~\cite{\citeTracers} --- not only incur ETL costs, but create rank-major layouts that
        fragment a single timestep across thousands of per-rank logs.
...
% the trace regardless of the workload duration (see \Cref{fig:des-arch}).
...
\begin{itemize}[leftmargin=*]
  \item Aggregators must issue RDMA GETs at controlled concurrency to prevent catastrophic
        throughput collapse, as NICs have limited resources for tracking active RDMA operations (1--2 on older fabrics).
        The pull-based model, which resembles receiver-driven flow control, lets aggregators stay in
        control of data volume and is essential for a stable tree-based incast topology.
...
\Cref{fig:des-twopc} illustrates the protocol via three cases.
...
% As a result, TS2PC enables a control plane that is atomic, timestep-consistent, and asynchronous in
% the common case. 
