...
TAU~\cite{Shend2006:TAU}, \scorep{}~\cite{Knupf2012:Score-P}, Caliper~\cite{Boehm2016:Caliper}, and
...
\begin{itemize}[leftmargin=*]
  ...
  \item At 4096 ranks, ORCA transfers 1.8\,GB/s of data at under 2\% overhead while sharing the fabric with the application without QoS support (550\,GB on-wire, 111\,GB after compression, over 308\,s).
        ...
        %         communication.
  \item Marginal increase in overhead between the lightweight and detailed profiles at 4096 ranks is only
...
frequently crashed with out-of-memory errors even with 64\,MB trace buffers.
...
\begin{itemize}[leftmargin=*]
  ...
  \item Differences can be partly explained by format efficiency --- per-record footprint of OTF2, being a binary
        format, is only 1.4$\times$ larger, while that of plaintext formats is 7--13$\times$ larger.
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Flush-induced jitter, especially with \scorep{} and Caliper, creates stragglers that other
        ranks wait on by polling --- generating additional events while waiting.
\end{enumerate}
...
ORCA queries use unmodified Polars~\cite{:Polars} with no partition awareness for simplicity--- a
...
\parahead{Why ORCA Works}
Plaintext formats used by analytics-oriented tracers --- \dftracer{} and Caliper --- require
expensive ETL to materialize dataframes before any analyses, while Parquet enables zero-ETL
post-hoc analytics.
...
% post-hoc analytics.
Additionally, ORCA's timestep-partitioned layout enables both file-level pruning and intra-file
pruning via rowgroup statistics.
...
Switching from \texttt{verbs} to the libfabric \texttt{TCP} provider increases ORCA's overhead from
...
The first demonstrates a monitoring workflow: measuring the count of \mpiw{} exceeding
1\,ms each timestep, while the second persists outlier events to Parquet.
...
While ORCA was designed for human operators, the emergence of LLM-based coding agents---capable of issuing commands and analyzing
outputs---offers a way to validate the steerability paradigm.
We task an LLM agent with localizing the anomaly from the AMR code at 4096
ranks (\Cref{fig:orca:bg-volatile}).
...
\begin{enumerate}[leftmargin=*]
  ...
  \item At 510\,s, it used the collected Kokkos events to identify a \texttt{map.clear()} in AMR redistribution as the culprit.
  ...
  \item At 1335\,s, telemetry confirmed waits as the root cause.
        LLM reported the complete call flow at 1605\,s. Application then resumed with baseline collective monitoring (\flowmark{1}).
...
User interventions were minor: rejecting an incomplete explanation and asking for
basic filtering.
While the exercise relied on pre-existing annotations in relevant region, dynamic
instrumentation~\cite{:eBPF, Berna2011:DynInst} would enable investigations to
continue into the MPI and fabric layers.
