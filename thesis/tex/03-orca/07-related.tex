...
\begin{table}[h]
  ...
  \begin{tabularx}{1\textwidth}{p{1.0in} p{1.5in} Y Y}
    ...
    \midrule
    \multirow[t]{1}{=}[0pt]{\raggedright XPUTimer~\cite{Cui2025:XPUTimer} \emph{(Training anomalies)}} & Stall detection/disambiguation   & Call stack analysis                                                  & In-situ heartbeat reduction                                                                   \\
    \addlinespace
                                                                                                       & Slowdown analysis                & Periodic metrics                                                     & In-situ trace event aggregation                                                               \\
    \midrule
    TrainCheck~\cite{Jiang2025:TrainCheck} \emph{(Training correctness)}                               & Correctness invariants           & Offline trace analysis for invariant generation,  online enforcement & Timestep-aligned traces for offline generation, in-situ enforcement via OrcaFlow              \\
    \midrule
    \multirow[t]{1}{=}[0pt]{\raggedright Minder~\cite{Deng2025:Minder} \emph{(Fault~diagnosis)}}       & Fail-slow hardware               & Cluster-level metrics and anomaly detection                          & Workload-level trace events                                                                   \\
    \midrule
    \multirow[t]{1}{=}[0pt]{\raggedright Aegis~\cite{Dong2025:Aegis} \emph{(Fault~diagnosis)}}         & Fail-stop hardware               & Per-node NCCL counters pulled on timeouts                            & Workload-level trace events                                                                   \\
    \midrule
    \multirow[t]{2}{=}[0pt]{\raggedright Holmes~\cite{Yao2025:Holmes} \emph{(Fail-slows)}}             & Outlier collective detection     & Random forest model                                                  & Policy-dependent                                                                              \\
    \addlinespace
                                                                                                       & Localization                     & Graph search                                                         & Learn collective topology at bootstrap, \texttt{GROUPBY swid}, trace critical path in reverse \\
    \midrule
    ...
    \addlinespace
                                                                                                       & Online interventions             & Tune micro-batch distribution or parallelism topology                & Supported via control plane                                                                   \\
    \bottomrule
  \end{tabularx}
  \caption{Comparison of diagnostic approaches in ML training.}~\label{tab:orca:related-work}
\end{table}
