...
\begin{table}[h]
  ...
  \begin{tabularx}{1\textwidth}{p{1.0in} p{1.5in} Y Y }
    ...
    \midrule
    \multirow[t]{2}{=}[0pt]{\raggedright Greyhound~\cite{Wu2025:GREYHOUND} \emph{(Fail-slows)}}
    ...
    control plane                                                                                                                                                                                                                                                         \\
    \midrule
    \multirow[t]{1}{=}[0pt]{\raggedright XPUTimer~\cite{Cui2025:XPUTimer} \emph{(Training anomalies)}}
    ...
    \multirow[t]{2}{=}[0pt]{\raggedright Holmes~\cite{Yao2025:Holmes} \emph{(Fail-slows)}}       & Outlier
    ...
    \midrule
    \multirow[t]{1}{=}[0pt]{\raggedright Minder~\cite{Deng2025:Minder} \emph{(Fault~diagnosis)}} &
    Fail-slow hardware                                                                           & Cluster-level metrics and anomaly detection   & Workload-level trace events                                                                                            \\
    \midrule
    \multirow[t]{1}{=}[0pt]{\raggedright Aegis~\cite{Dong2025:Aegis} \emph{(Fault~diagnosis)}}   &
    Fail-stop hardware                                                                           & Per-node NCCL counters pulled on timeouts     & Workload-level trace events                                                                                            \\
    \midrule
    Reveal~\cite{Chen2025:Reveal} \emph{(Training~anomalies)}                                    & Anomaly detection and bottleneck
    attribution                                                                                  & Time-windowed metrics + unsupervised learning & Workload-level trace events                                                                                      \\
    \bottomrule
  \end{tabularx}
  \caption{Comparison of diagnostic approaches in ML training.
...
% \parahead{Towards Unified Analytics} ADIOS~\cite{Eisen2024:ADIOS} enables in-situ analytics via a
...
% \parahead{Streaming Analytics} ADIOS~\cite{Eisen2024:ADIOS} enables in-situ analytics via a
...
% -- "observability monitoring and in situ analytics in HPC" \cite{Eisen2024:ADIOS} -- streaming in
...
% ADIOS \cite{Godoy2020:ADIOS2,Eisen2024:ADIOS} interesting architectural comparison wrt ORCA. Many
...
% % ML Papers: % Aegis (Alibaba): no access to source code, instrument CCL % - Pull local stats if
% timeout detected (timeout ~ only fail-slow) % - If collectives started == n, it's a communication
% failure. If it's n - 1, it's a compute failure.
