...
this is the natural unit of in-situ analysis, and maps directly to \emph{(timestep, rank)}-partitioned
Parquet for offline access.
...
\begin{enumerate}
  \item A synthesis of BSP efficiency bottlenecks as runtime-unknowable conditions whose impact is
        governed by feedback-loop latency.
  \item CARP, an adaptive in-situ range partitioning mechanism that replaces costly post-hoc sorting
        with a runtime repartitioning loop driven by reconstructed global state.
  \item A telemetry-driven diagnostic workflow for AMR performance analysis, and CPLX, a tunable
        placement policy that manages locality/load trade-offs under runtime variability.
  \item ORCA, an agent-agnostic, declarative observability and control substrate for closing adaptive
        loops in tightly coupled parallel systems.
\end{enumerate}
