...
% addressable.
% In BSP, synchronization boundaries are the moments when distributed execution becomes globally
% interpretable and when coordinated change is well-defined.
% Closing a runtime feedback loop therefore requires two capabilities at those boundaries:
% observation to materialize a view of execution state, and action to apply a coordinated response.
% % The state could be telemetry or application structures, and actions could range from
% % additional views to interventions.
% Together, these enable always-on visibility into runtime behavior: lightweight workload-level
% metrics in the common case, and higher-fidelity views and coordinated interventions
% in response to observed anomalies.
...
% We propose ORCA (\emph{Observability with Real-time Control and Aggregation}), which instantiates
...
% \begin{tcolorbox}[thesisbox]
  \tstatement{\textbf{Thesis Statement:} \emph{A large class of efficiency bottlenecks in BSP systems arise from
...
The real cost is foregone analysis: feedback bottlenecks incentivize ad-hoc mitigations
over systematic investigation.
CARP shows that online closure is feasible, but implements a fixed-function loop built for one
adaptive mechanism, not as general-purpose programmable infrastructure.
...
\begin{itemize}
  \item \textbf{In-situ view materialization.
        ...
        always-on lightweight aggregates in the common case, with details available on demand.
...
ORCA is agnostic to how telemetry is produced and to who drives the loop---
